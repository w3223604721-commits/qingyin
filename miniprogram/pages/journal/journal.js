...
Page({
  ...
  chooseCover() {
    wx.chooseImage({
      ...
      success: (res) => {
        this.setData({ 'form.coverPhoto': res.tempFilePaths[0] })
      }
    })
  },
  ...
  saveJourney() {
    ...
    if (!form.name.trim()) {
      wx.showToast({ title: '请输入旅程名称', icon: 'none' })
      return
    }
    ...
    wx.nextTick(() => {
      try {
        ...
        console.error('saveJourney error:', e)
        wx.hideLoading()
  ...
  // ====== 菜单 ======
  showMenu(e) {
    this.setData({ showMenuModal: true, menuId: e.currentTarget.dataset.id })
  },
  ...
  editJourney() {
    ...
    if (journey) {
      this.setData({
        ...
        form: { name: journey.name, city: journey.city, startDate: journey.startDate, endDate: journey.endDate, desc: journey.desc, coverPhoto: journey.coverPhoto }
      })
    }
  },
  deleteJourney() {
    ...
    wx.showModal({
      ...
      success: (res) => {
        if (res.confirm) {
          ...
          if (idx !== -1) {
            const [removed] = data.journeys.splice(idx, 1)
  ...
  goDetail(e) {
    ...
    wx.navigateTo({ url: `/pages/journey-detail/journey-detail?id=${id}` })
  },
